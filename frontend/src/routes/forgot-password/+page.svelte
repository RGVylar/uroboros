...
<div style="display:flex; flex-direction:column; justify-content:center; min-height:85dvh; padding:1.25rem;">
	<div style="text-align:center; margin-bottom:2rem;">
		...
		<h1 style="font-size:1.75rem; color:#fff; letter-spacing:-0.05em; font-family:'Lora','Georgia',serif; font-weight:400; margin:0;">{t('forgot.title')}</h1>
	</div>
	...
	{#if sent}
		<div style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.09); border-radius:22px; padding:1.5rem; text-align:center;">
			...
			<a href="/login" style="display:inline-block; margin-top:1.25rem; color:oklch(85% 0.15 160); font-size:0.875rem; font-weight:600; text-decoration:none;">{t('forgot.backToLogin')}</a>
		</div>
	{:else}
		<div style="background:rgba(255,255,255,0.05); backdrop-filter:blur(24px) saturate(160%); border:1px solid rgba(255,255,255,0.09); border-radius:22px; padding:1.25rem;">
			...
			<form onsubmit={(e) => { e.preventDefault(); submit(); }}>
				<div style="margin-bottom:1rem;">
					<label for="email" style="display:block; font-size:0.6875rem; font-weight:600; color:rgba(255,255,255,0.6); text-transform:uppercase; letter-spacing:0.05em; margin-bottom:0.375rem;">{t('forgot.email')}</label>
					<input id="email" type="email" bind:value={email} placeholder={t('login.emailPlaceholder')} required style="width:100%; box-sizing:border-box;" />
				</div>
		...
		<a href="/login" style="display:block; text-align:center; margin-top:1rem; color:rgba(255,255,255,0.45); font-size:0.8125rem; text-decoration:none;">{t('forgot.backToLogin')}</a>
	{/if}
</div>
